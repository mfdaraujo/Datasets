...
metrics.to_csv('metrics_verification4.csv', index=False)
user = np.arange(userMax)
data = np.array([user, salva]).T
dfThreshold = pd.DataFrame(data= data, columns=['user','threshold'])

dfThreshold.to_csv('Threshold4.csv', index=False)
